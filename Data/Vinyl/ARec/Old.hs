...
instance ToARec us => ToARec (u ': us) where
  aRecValues (x :& xs) = unsafeCoerce x : aRecValues xs
  {-# INLINE aRecValues #-}
...
toARecFast rs =
  ...
  in ARec $! Array.listArray (0, n-1) (aRecValues rs)
{-# INLINE toARecFast #-}
...
newtype MkARec f us = MkARec (Int -> (Int, [Any]))

infixr 1 &:
(&:) :: f u -> MkARec f us -> MkARec f ( u ': us )
(&:) !x !(MkARec fxs) = MkARec (\i -> let (l, xs) = fxs (i + 1)
                                    in (l, unsafeCoerce x : xs))
{-# INLINE (&:) #-}

arnil :: MkARec f '[]
arnil = MkARec (\i -> (i - 1, []))
-- We don't add an element
-- , so the last index is one less
{-# INLINE arnil #-}

arec :: MkARec f us -> ARec f us
arec (MkARec f) = let (!len, !elems) = f 0
                  in ARec $ Array.listArray (0, len) elems
{-# INLINE arec #-}
